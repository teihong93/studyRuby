...
def myFirstFunction(arg)
  "Hello " + arg
  #루비는 마지막으로 실행된 표현식의 결과값을 리턴함.
end

puts myFirstFunction("world")
#=> Hello world

## 루비의 표현식 보간
# 루비는 #{}를 통해 표현식보간을 사용할 수 있다.

def myFirstExpression(arg)
  "Hello #{arg.capitalize}"
end

puts myFirstExpression("world")
#=> Hello World
...
# 리터럴 방식의 배열 선언
myFirstArray = ["this","is","my","first","ruby","array"]
# 단축 문법을 사용한 배열 선언
shortenedArray = %w{this is my first ruby array}
...
myFirstShortenedHash = {
    name:'tei',
    language: 'ruby',
    level:'low'
}

puts myFirstSymbolHash[:name]
#=> tei
...
whoSaysWhat {|person,what| puts "#{person} says #{what}"}
#=> tei says hello (...)
